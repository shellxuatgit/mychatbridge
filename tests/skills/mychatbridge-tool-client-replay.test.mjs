...
const script = 'skills/mychatbridge-tool-client-replay/scripts/replay-client-fixture.mjs'

test('client profiles define prompt and tools expectations', () => {
  const cherry = JSON.parse(fs.readFileSync('skills/mychatbridge-tool-client-replay/profiles/cherry-studio.json', 'utf8'))
  ...
  const openai = JSON.parse(fs.readFileSync('skills/mychatbridge-tool-client-replay/profiles/openai-tools.json', 'utf8'))
...
test('replay dry-run reports selected fixture and model', () => {
  const result = spawnSync('node', [script, '--fixture', 'sample.json', '--profile', 'cherry-studio', '--model', 'deepseek-v4-flash', '--dry-run'], {
    env: {
      ...process.env,
      MYCHATBRIDGE_API_KEY: 'sk-secret-value',
    },
    encoding: 'utf8',
  })
...
test('replay dry-run reports schema details when fixture exists without requiring api key', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'mychatbridge-replay-'))
...
`)
  ...
  const result = spawnSync('node', [script, '--fixture', fixture, '--profile', 'cherry-studio', '--model', 'deepseek-v4-flash'], {
    env: {
      ...
      MYCHATBRIDGE_BASE_URL: 'http://127.0.0.1:8080',
      MYCHATBRIDGE_API_KEY: 'runtime-secret',
    },
    encoding: 'utf8',
  })
...
test('missing model exits nonzero without leaking key', () => {
  ...
  assert.match(result.stderr, /--model or MYCHATBRIDGE_MODEL is required/)
