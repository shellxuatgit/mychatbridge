...
const script = 'skills/mychatbridge-management-api/scripts/management-api.mjs'
const skill = 'skills/mychatbridge-management-api/SKILL.md'
...
test('management helper dry-run commands do not need network and do not leak secret', () => {
  ...
  for (const command of commands) {
    const result = spawnSync('node', [script, ...command], {
      env: {
        ...process.env,
        MYCHATBRIDGE_BASE_URL: 'http://127.0.0.1:1',
        MYCHATBRIDGE_MGMT_SECRET: 'mgmt_super_secret_value',
      },
      encoding: 'utf8',
    })
...
`)
  ...
  try {
    const result = await runScript([
      ...
      MYCHATBRIDGE_BASE_URL: 'http://mock.local',
      MYCHATBRIDGE_MGMT_SECRET: 'mgmt_super_secret_value',
    }, {
