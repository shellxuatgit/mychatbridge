...
const script = 'skills/mychatbridge-provider-model-matrix/scripts/run-model-matrix.mjs'
const scriptPath = path.resolve(script)

function makeTempRunDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'mychatbridge-model-matrix-'))
...
test('matrix runner dry-run does not leak configured secrets', () => {
  const result = spawnSync('node', [script, '--fixture', 'fixture.json', '--dry-run'], {
    env: {
      ...process.env,
      MYCHATBRIDGE_API_KEY: 'sk_super_secret_value',
      MYCHATBRIDGE_MGMT_SECRET: 'mgmt_super_secret_value',
    },
    encoding: 'utf8',
  })
...
`)
  ...
  assert.match(output.reportPath, /^backup\/har\/mychatbridge-model-matrix-.+\.json$/)
