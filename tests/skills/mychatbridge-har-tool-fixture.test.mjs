...
const script = 'skills/mychatbridge-har-tool-fixture/scripts/extract-har-fixtures.mjs'
...
test('extracts sanitized tool fixture from HAR', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'mychatbridge-har-'))
...
test('uses env fallback, exact chat path selection, and body secret redaction', () => {
  ...
  const result = spawnSync('node', [script, '--out', out], {
    ...
    env: {
      ...process.env,
      MYCHATBRIDGE_HAR: har,
      MYCHATBRIDGE_CLIENT_PROFILE: 'env-client',
    },
  })
