...
test('Mimo query preserves managed tool call history for follow-up requests', () => {
  ...
  assert.match(query, /<\|MYCHATBRIDGE\|tool_calls>/)
  assert.match(query, /<\|MYCHATBRIDGE\|invoke name="weather-test:get_weather">/)
  assert.match(query, /<\|MYCHATBRIDGE\|tool_result tool_call_id="call_1">/)
...
test('Mimo stream converts managed XML into OpenAI tool calls', async () => {
  ...
  const stream = Readable.from([
    'event: message\n',
    'data: {"content":"<|MYCHATBRIDGE|tool_calls><|MYCHATBRIDGE|invoke name=\\"weather-test:get_weather\\"><|MYCHATBRIDGE|parameter name=\\"city\\"><![CDATA[Hangzhou]]></|MYCHATBRIDGE|parameter></|MYCHATBRIDGE|invoke></|MYCHATBRIDGE|tool_calls>"}\n\n',
  ])
  ...
  assert.doesNotMatch(output, /MYCHATBRIDGE/)
})
