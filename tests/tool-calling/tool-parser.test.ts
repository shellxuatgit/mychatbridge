...
test('managed xml parses valid MyChatBridge tool call', () => {
...
test('managed xml ignores fenced tool examples', () => {
  const result = managedXmlProtocol.parse(
    '```xml\n<|MYCHATBRIDGE|tool_calls><|MYCHATBRIDGE|invoke name="default_api:read_file"><|MYCHATBRIDGE|parameter name="filePath">fake</|MYCHATBRIDGE|parameter></|MYCHATBRIDGE|invoke></|MYCHATBRIDGE|tool_calls>\n```',
    { tools, protocol: 'managed_xml' },
  )
...
test('managed XML parser rejects undeclared tool names and records invalid names', () => {
  const result = managedXmlProtocol.parse(
    '<|MYCHATBRIDGE|tool_calls><|MYCHATBRIDGE|invoke name="missing_tool">{}</|MYCHATBRIDGE|invoke></|MYCHATBRIDGE|tool_calls>',
    { tools, protocol: 'managed_xml' },
  )
