...
test('MyChatBridge XML marker split across chunks emits a tool call', () => {
  const parser = new ToolStreamParser(plan('managed_xml'))
  assert.deepEqual(parser.push('<|MYCHATBRIDGE|tool_', baseChunk), [])
  const chunks = parser.push('calls><|MYCHATBRIDGE|invoke name="default_api:read_file"><|MYCHATBRIDGE|parameter name="filePath">/tmp/a</|MYCHATBRIDGE|parameter></|MYCHATBRIDGE|invoke></|MYCHATBRIDGE|tool_calls>', baseChunk)

  assert.equal(chunks.at(-1)?.choices[0].delta.tool_calls[0].function.name, 'default_api:read_file')
})

test('partial MyChatBridge start marker is reported as buffered so stream handlers do not leak it', () => {
  const parser = new ToolStreamParser(plan('managed_xml'))
  const chunks = parser.push('<|MYCHATBRIDGE|tool_calls', baseChunk)
