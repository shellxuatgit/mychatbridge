...
export class ProviderChecker {
  ...
  static async checkAccountToken(
    ...
    switch (provider.id) {
      ...
      case 'chatgpt':
      case 'chatgpt-web':
        return this.checkChatGPTCookie(account.credentials)
      default:
  ...
  private static async checkChatGPTCookie(credentials: Record<string, string>): Promise<TokenCheckResult> {
    const cookie = credentials.cookie || credentials.cookies || credentials.sessionCookie
    if (!cookie) {
      return { valid: false, error: 'ChatGPT cookie is required' }
    }
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        const data = response.data as { user?: { email?: string; name?: string }; accessToken?: string }
        return {
          valid: Boolean(data.user || data.accessToken),
          error: data.user || data.accessToken ? undefined : 'ChatGPT session is not authenticated',
          userInfo: data.user ? { email: data.user.email, name: data.user.name } : { name: 'ChatGPT User' },
        }
      }

      if (response.status === 401 || response.status === 403) {
        return { valid: false, error: `ChatGPT cookie is invalid or expired (HTTP ${response.status})` }
      }
      return { valid: false, error: `ChatGPT validation failed: HTTP ${response.status}` }
    } catch (error) {
      return {
        valid: false,
        error: error instanceof AxiosError ? `ChatGPT validation failed: ${error.message}` : 'ChatGPT validation failed',
      }
    }
  }
