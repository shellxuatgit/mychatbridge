...
import type { WebProviderCapabilities } from '../proxy/core/providerContract.ts'
import type { TokenUsage } from '../proxy/usage.ts'

/**
 * Error class identifiers (kept in sync with core/errorClassification.ts).
 * Duplicated here to avoid a circular import: types → fallbackGraph →
 * errorClassification → providerContract → types.
 */
...
/** What to do when an error class triggers a fallback. */
export type FallbackAction =
  | 'next_provider'
  | 'next_account_then_provider'
  | 'abort'
  | 'cooldown_and_retry'

/** Fallback graph configuration (smart failover routing). */
export interface FallbackGraphConfig {
  /** Whether the fallback graph is enabled */
  enabled: boolean
  /** Override nodes; merged with the default graph. */
  nodes?: Partial<Record<GatewayErrorClass, FallbackAction>>
  /** Max total fallback attempts across all nodes (default 3) */
  maxTotalRetries: number
}

export const DEFAULT_FALLBACK_CONFIG: FallbackGraphConfig = {
  enabled: true,
  maxTotalRetries: 3,
}
 ...
 * Provider Type Enum
 */
export type ProviderType = 'builtin' | 'custom' | 'web'
 ...
 * - browser: managed browser session (web provider, e.g. Chat Connect)
 */
...
export interface BuiltinProviderConfig extends Omit<Provider, 'createdAt' | 'updatedAt'> {
  ...
  /** Official logo URL for network loading with fallback */
  logoUrl?: string
}

/**
 * Load Balance Strategy Enum
 * 'weighted' is the gateway router extension (see proxy/core/router).
 */
...
export interface Account {
  ...
  /** Today used count */
  todayUsed?: number
  /** Routing weight for the 'weighted' router strategy (defaults to 1) */
  weight?: number
  /** Max concurrent requests for this account (optional, future quota layer) */
  concurrencyLimit?: number
  /**
   * Relative cost per request for cost-aware routing (1 = baseline).
   * Higher = more expensive; cost-aware routing prefers lower cost.
   */
...
export interface Provider {
  ...
  /** Capability overrides (partial; merged over the capability registry) */
  capabilities?: Partial<WebProviderCapabilities>
}
...
export interface ModelMapping {
  ...
  /** Required capabilities (capability-aware routing) */
  capabilities?: Partial<WebProviderCapabilities>
}
...
export interface AppConfig {
  ...
  /** Quota / rate-limit configuration (gateway quota layer) */
  quotaConfig: QuotaConfig
  /** Fallback graph configuration (smart failover routing) */
  fallbackGraph: FallbackGraphConfig
}

/**
 * Quota / Rate Limit Configuration
 * Multi-dimensional quotas (API key / account / provider / per-minute) plus
 * optional rate limiting (RPM / TPM / concurrency) for the gateway layer.
 */
export interface QuotaConfig {
  /** Whether the quota/rate-limit layer is enabled */
  enabled: boolean
  /** Per-minute request cap per API key (RPM); 0 = unlimited */
  rpmPerApiKey: number
  /** Requests per day per API key; 0 = unlimited */
  rpdPerApiKey: number
  /** Concurrent requests per API key; 0 = unlimited */
  concurrencyPerApiKey: number
  /** Requests per day per account; 0 = unlimited */
  rpdPerAccount: number
  /** Requests per day per provider; 0 = unlimited */
  rpdPerProvider: number
  /** Requests per day per client IP; 0 = unlimited */
  rpdPerIp: number
}

/**
 * Default Quota Configuration (layer off by default; opt-in)
 */
...
export interface RequestLogEntry {
  ...
  /** Token usage reported by the provider (absent when unavailable) */
  usage?: {
    promptTokens: number
...
export interface DailyStatistics {
  ...
  /** Provider token usage (absent in legacy persisted stats) */
  providerTokens?: Record<string, TokenUsage>
}
...
export const DEFAULT_CONFIG: AppConfig = {
  ...
  fallbackGraph: DEFAULT_FALLBACK_CONFIG,
}
