...
// Electron APIs are only available inside the Electron main process. Outside a
// bundled runtime (e.g. plain Node tests) the import resolves to stubs with
// undefined members, which is why we use a lazy getter for encryption.
...
import { addUsage, EMPTY_TOKEN_USAGE, type TokenUsage } from '../proxy/usage.ts'
...
import { migrateLegacyStorage } from './migrate.ts'
...
class StoreManager {
  ...
  private statsNotifyTimer: NodeJS.Timeout | null = null

  private notifyStatsUpdated(stats: PersistentStatistics): void {
    if (this.statsNotifyTimer) return
    this.statsNotifyTimer = setTimeout(() => {
      this.statsNotifyTimer = null
      try {
        const payload = this.getStatistics()
        if (this.mainWindow && !this.mainWindow.isDestroyed()) {
          this.mainWindow.webContents.send('statistics:updated', payload)
        }
      } catch {
  ...
  async initialize(): Promise<void> {
    ...
    // Migrate legacy ~/.chat2api data before the new store opens its files
  ...
  /**
   * Get Storage Path
   * Storage path: ~/.mychatbridge/
   *
   * Data in the legacy location (~/.chat2api/) is migrated on first run —
   * see ensureStorageMigrated(). The encryption key is intentionally kept
   * fixed across the rename so migrated data stays decryptable.
   */
  private getStoragePath(): string {
    return join(homedir(), '.mychatbridge')
  }

  /**
   * One-time data migration from ~/.chat2api/ (legacy) to ~/.mychatbridge/.
   *
   * Copied before any electron-store handle opens a file inside the new
   * directory, and only when the new directory does not already exist
   * (fresh installs and already-migrated installs are untouched). The
   * legacy directory is preserved as a backup and never deleted.
   */
  private async ensureStorageMigrated(): Promise<void> {
    const legacyPath = join(homedir(), '.chat2api')
    const newPath = this.getStoragePath()
    const result = migrateLegacyStorage(legacyPath, newPath)

    if (result.migrated) {
      console.log('[Store] Storage migration complete (legacy directory kept as backup)')
    } else if (result.reason === 'failed') {
      console.warn('[Store] Storage migration failed; starting with a fresh store')
    }
  }

  /**
   * Lazy-resolve Electron safeStorage. In the Electron main process this is
   * the real module; outside of it (Node tests) it is undefined, and callers
   * fall back to plain (unencrypted) JSON storage.
   */
  ...
  private async initializeDefaultProviders(): Promise<void> {
    ...
    // Backfill any builtin providers missing from the persisted list so all
    // registered providers show up on the Home page.
    const existingIds = new Set(validProviders.map((p: Provider) => p.id))
    const now = Date.now()
    for (const builtin of BUILTIN_PROVIDERS) {
      ...
      validProviders.push({
        ...
        credentialFields: builtin.credentialFields,
      })
      existingIds.add(builtin.id)
      console.log('[Store] Backfilled missing builtin provider:', builtin.id)
    }
  ...
  recordRequestInStats(
    ...
    if (providerId && usage) {
      newStats.providerTokens = addUsage(newStats.providerTokens, providerId, usage)
    }
    ...
    if (providerId && usage) {
      newStats.dailyStats[today] = {
        ...newStats.dailyStats[today],
        providerTokens: addUsage(newStats.dailyStats[today].providerTokens || {}, providerId, usage),
      }
    }
    
    this.store!.set('statistics', newStats)
    this.notifyStatsUpdated(newStats)
    return newStats
  }

  /**
   * Record token usage for a provider (e.g. streaming usage arriving at stream end)
   */
  recordProviderTokenUsage(providerId: string, usage: TokenUsage): void {
    this.ensureInitialized()
    const stats = this.store!.get('statistics') || DEFAULT_STATISTICS
    const today = new Date().toISOString().split('T')[0]
    ...
    const newStats: PersistentStatistics = {
      ...stats,
      lastUpdated: Date.now(),
      providerTokens: addUsage(stats.providerTokens || {}, providerId, usage),
      dailyStats: {
        ...stats.dailyStats,
        [today]: {
          ...daily,
          providerTokens: addUsage(daily.providerTokens || {}, providerId, usage),
        },
      },
    }
    this.store!.set('statistics', newStats)
