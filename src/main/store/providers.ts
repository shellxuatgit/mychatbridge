...
export class ProviderManager {
  /**
   * API-first provider list.
   * Legacy browser/web providers remain in storage for migration compatibility,
   * but are no longer exposed by the active provider management API.
   */
  private static getApiProviders(): Provider[] {
    return storeManager.getProviders().filter((p) => p.type !== 'web')
  }
  ...
  static getById(id: string): Provider | undefined {
    ...
    return provider?.type === 'web' ? undefined : provider
  }
  ...
  static getEnabled(): Provider[] {
    return this.getApiProviders().filter((p) => p.enabled)
  }
  ...
  static getByType(type: ProviderType): Provider[] {
    if (type === 'web') return []
    return this.getApiProviders().filter((p) => p.type === type)
  }
  ...
  static getByAuthType(authType: AuthType): Provider[] {
    return this.getApiProviders().filter((p) => p.authType === authType)
  }
  ...
  static create(data: {
    ...
    if (data.type === 'web' || data.authType === 'browser') {
      throw new Error('Web/browser providers are no longer supported; use an API provider instead')
    }

    const existing = this.getApiProviders()
  ...
  static resetBuiltinProviders(): void {
    ...
    const now = Date.now()
    const defaultBuiltin: Provider[] = BUILTIN_PROVIDERS
      .filter((p) => p.type !== 'web' && p.authType !== 'browser')
      .map((p) => ({
        ...p,
        createdAt: now,
        updatedAt: now,
    ...
    storeManager.addLog('info', 'Reset built-in provider configuration (API-only)')
  }
