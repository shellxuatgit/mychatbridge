...
import { probeOpenAiApi } from '../proxy/apiProbe'
...
import { handleConnect, handleImportBrowser, handleHealth, handlePlaywrightLoginConfig, handlePlaywrightLogin, handleCancelPlaywrightLogin, handleSaveImportedSession } from '../webRuntime/webRuntimeHandlers'
import { handleAutoConnect, handleCheckSession } from '../webRuntime/autoConnect'
import { CookieImporter } from '../webRuntime/cookieImporter'
import { LocalStorageReader } from '../webRuntime/localStorageReader'
import { DEEPSEEK_ORIGIN } from '../webRuntime/credentialSources'
...
const lsReader = new LocalStorageReader()
...
export async function registerIpcHandlers(mainWindow: BrowserWindow | null): Promise<void> {
  ...
  ipcMain.handle(IpcChannels.PROXY_TEST_CONNECTION, async (_, options?: { model?: string; apiKey?: string }) => {
    const config = storeManager.getConfig()
    const providerModels = storeManager.getProviders().flatMap((provider) => provider.supportedModels || [])
    const model = options?.model || providerModels[0] || 'gpt-4o'
    return probeOpenAiApi({
  ...
  ipcMain.handle(IpcChannels.PROVIDERS_CHECK_STATUS, async (_, providerId: string): Promise<ProviderCheckResult> => {
    ...
    const logs: Array<{ timestamp: number; level: 'info' | 'error'; message: string; messageKey: string; messageParams: Record<string, unknown> }> = [
      {
        timestamp: startedAt,
        level: 'info',
        message: `开始测试 Provider：${providerId}`,
        messageKey: 'providers.testLogs.start',
        messageParams: { providerId },
      },
    ]
    ...
    if (!provider) {
      logs.push({
        ...
        messageKey: 'providers.testLogs.providerNotFound',
        messageParams: {},
      })
      return {
        providerId,
    ...
    logs.push({
      timestamp: Date.now(),
      level: 'info',
      message: `开始连接：${provider.apiEndpoint}`,
      messageKey: 'providers.testLogs.connect',
      messageParams: { endpoint: provider.apiEndpoint },
    })
    ...
    if (result.status === 'online') {
      logs.push({
        timestamp: Date.now(),
        level: 'info',
        message: `测试成功，耗时 ${elapsedMs}ms`,
        messageKey: 'providers.testLogs.success',
        messageParams: { ms: elapsedMs },
      })
    } else {
      logs.push({
        timestamp: Date.now(),
        level: 'error',
        message: `测试失败，耗时 ${elapsedMs}ms${result.error ? `：${result.error}` : ''}`,
        messageKey: 'providers.testLogs.failed',
        messageParams: { ms: elapsedMs, error: result.error || '' },
      })
    }
  ...
  ipcMain.handle(IpcChannels.ACCOUNTS_VALIDATE_TOKEN, async (_, providerId: string, credentials: Record<string, string>) => {
    ...
    const logs: Array<{ timestamp: number; level: 'info' | 'error'; message: string; messageKey: string; messageParams: Record<string, unknown> }> = [
      {
        timestamp: startedAt,
        level: 'info',
        message: `开始校验账号凭证：${providerId}`,
        messageKey: 'providers.testLogs.validateStart',
        messageParams: { providerId },
      },
    ]
    ...
    if (!provider) {
      ...
      return { valid: false, error: 'Provider not found', logs }
    }

    logs.push({
      timestamp: Date.now(),
      level: 'info',
      message: `已找到 Provider：${provider.name}`,
      messageKey: 'providers.testLogs.providerFound',
      messageParams: { name: provider.name },
    })
    ...
    if (result.valid) {
      logs.push({
        timestamp: Date.now(),
        level: 'info',
        message: `校验成功，耗时 ${elapsedMs}ms`,
        messageKey: 'providers.testLogs.validateSuccess',
        messageParams: { ms: elapsedMs },
      })
    } else {
      logs.push({
        timestamp: Date.now(),
        level: 'error',
        message: `校验失败：${result.error || '未知错误'}，耗时 ${elapsedMs}ms`,
        messageKey: 'providers.testLogs.validateFailed',
        messageParams: { ms: elapsedMs, error: result.error || '未知错误' },
      })
    }
    return { ...result, logs }
  })
  ...
  ipcMain.handle(IpcChannels.APP_SIMULATE_UPDATE, async (_, targetVersion?: string) => {
    await updaterManager.simulateUpdate(targetVersion)
    return updaterManager.getStatus()
  })
  ...
  // ==================== Web Runtime Handlers ====================

  ipcMain.handle('webRuntime:connect', async (_, params: { providerId: string; accountId: string }) => {
    console.error(`[diag] webRuntime:connect enter provider=${params.providerId}`)
    try {
      const result = await handleConnect(params)
      console.error('[diag] webRuntime:connect exit ok')
      return result
    } catch (error) {
      console.error('[diag] webRuntime:connect threw:', error)
      throw error
    }
  })

  ipcMain.handle('webRuntime:importBrowser', async (_, params: { browser?: 'chrome' | 'edge' }) => {
    console.error(`[diag] webRuntime:importBrowser enter browser=${params?.browser ?? 'chrome'}`)
    try {
      const result = await handleImportBrowser(params)
      console.error(`[diag] webRuntime:importBrowser exit count=${result.length}`)
      return result
    } catch (error) {
      console.error('[diag] webRuntime:importBrowser threw:', error)
      throw error
    }
  })

  ipcMain.handle('webRuntime:autoConnect', async (_, providerId: string) => {
    console.error(`[diag] webRuntime:autoConnect enter providerId=${providerId}`)
    try {
      const result = await handleAutoConnect(providerId, {
        opener: (url) => {
          console.error(`[diag] autoConnect opener -> ${url}`)
          return shell.openExternal(url)
        },
        importer: new CookieImporter(),
        localStorageReader: (browser, tokenKey) => {
          console.error(`[diag] autoConnect localStorageReader browser=${browser} key=${tokenKey}`)
          return lsReader.findToken(browser, tokenKey, DEEPSEEK_ORIGIN)
        },
      })
      console.error(`[diag] webRuntime:autoConnect exit status=${(result as { status?: string })?.status}`)
      return result
    } catch (error) {
      console.error('[diag] webRuntime:autoConnect threw:', error)
      throw error
    }
  })

  ipcMain.handle('webRuntime:checkSession', async (_, providerId: string) => {
    console.error(`[diag] webRuntime:checkSession enter providerId=${providerId}`)
    try {
      const result = await handleCheckSession(providerId, {
        opener: (url) => {
          console.error(`[diag] checkSession opener -> ${url}`)
          return shell.openExternal(url)
        },
        importer: new CookieImporter(),
        localStorageReader: (browser, tokenKey) => {
          console.error(`[diag] checkSession localStorageReader browser=${browser} key=${tokenKey}`)
          return lsReader.findToken(browser, tokenKey, DEEPSEEK_ORIGIN)
        },
      })
      console.error(`[diag] webRuntime:checkSession exit status=${(result as { status?: string })?.status}`)
      return result
    } catch (error) {
      console.error('[diag] webRuntime:checkSession threw:', error)
      throw error
    }
  })

  ipcMain.handle('webRuntime:health', async () => {
    return handleHealth()
  })

  ipcMain.handle('webRuntime:playwrightLoginConfig', async () => {
    return handlePlaywrightLoginConfig()
  })

  ipcMain.handle('webRuntime:playwrightLogin', async (_, params: { providerId: string }) => {
    console.error(`[diag] webRuntime:playwrightLogin enter providerId=${params?.providerId}`)
    try {
      const result = await handlePlaywrightLogin(params)
      console.error(`[diag] webRuntime:playwrightLogin exit status=${result.status}`)
      return result
    } catch (error) {
      console.error('[diag] webRuntime:playwrightLogin threw:', error)
      throw error
    }
  })

  ipcMain.handle('webRuntime:cancelPlaywrightLogin', async (_, params: { providerId: string }) => {
    await handleCancelPlaywrightLogin(params)
    return { ok: true as const }
  })

  ipcMain.handle('webRuntime:saveImportedSession', async (_, params: { providerId: string; cookies: Array<Record<string, unknown>> }) => {
    console.error(`[diag] webRuntime:saveImportedSession enter providerId=${params?.providerId}`)
    return handleSaveImportedSession(params)
  })
