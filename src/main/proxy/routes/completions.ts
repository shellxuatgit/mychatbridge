...
import { normalizeUsage, parseSseUsage, estimateUsage, extractSseText } from '../usage'
    ...
    if (request.stream && result.stream) {
      ...
      transformStream.once('end', () => {
        const usage = parseSseUsage(collectedContent)
          ?? estimateUsage(String(request.prompt ?? ''), extractSseText(collectedContent))
      ...
      const usage = normalizeUsage((result.body as { usage?: unknown } | undefined)?.usage)
        ?? estimateUsage(String(request.prompt ?? ''), '')
      storeManager.recordRequestInStats(true, latency, request.model, provider.id, account.id, usage)
    }
  } catch (error) {
    const latency = Date.now() - startTime
    proxyStatusManager.recordRequestFailure(latency)
