...
/**
 * Normalize the stored cookie credential into a name → value map.
 * Accepts: a plain map, a JSON object string, or a JSON array of
 * `{ name, value }` records (the shape produced by cookiesToPayload).
 */
function normalizeStoredCookies(raw: unknown): StoredCookies {
  if (!raw) return {}
  if (typeof raw === 'object' && !Array.isArray(raw)) return { ...(raw as StoredCookies) }
  if (typeof raw === 'string') {
    try {
      return normalizeStoredCookies(JSON.parse(raw))
    } catch {
      return {}
    }
  }
...
export class PerplexityAdapter {
  ...
  constructor(provider: Provider, account: Account) {
    ...
    this.allCookies = normalizeStoredCookies(account.credentials.cookies)
