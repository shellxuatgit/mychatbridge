...
import {
  ...
  AccountDetail,
  ProviderFilter,
  WebProviderActionsDialog,
} from '@/components/providers'
import type { WebProviderAction, WebProviderBrowser } from '@/components/providers/WebProviderActionsDialog'
...
export function Providers() {
  ...
  const [webActionDialog, setWebActionDialog] = useState<{
    providerId: string
    action: WebProviderAction
    busy: boolean
    busyLabel?: string
  } | null>(null)
  const webActionProviderName = webActionDialog
    ? store.getProviderById(webActionDialog.providerId)?.name || ''
    : ''
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.electronAPI?.statistics?.onUpdated?.((stats) => {
      if (stats?.providerTokens) {
        setProviderTokens(stats.providerTokens)
      }
    })
  ...
  const handleWebActionClick = (providerId: string, action: WebProviderAction) => {
    setWebActionDialog({ providerId, action, busy: false })
  }

  const closeWebActionDialog = () => {
    if (webActionDialog?.busy) return
    setWebActionDialog(null)
  }

  /**
   * Web provider: launch the managed Chromium login. Requires at least one
   * account so the sidecar can persist the logged-in session against it
   * (account_id drives web-runtime session.save). Returns the accountId the
   * Connect session was bound to.
   */
  const handleWebConnect = async (providerId: string): Promise<string> => {
    const provider = store.getProviderById(providerId)
    if (!provider) return ''
    if (webActionDialog) {
      setWebActionDialog({ ...webActionDialog, busy: true, busyLabel: t('providers.connecting') })
    }
    ...
    if (providerAccounts.length === 0) {
      ...
      store.updateAccountCount(providerId, 1, 0)
    } else {
      const active = providerAccounts.find(a => a.status === 'active')
      accountId = (active || providerAccounts[0]).id
    }

    try {
      toast({
        title: t('providers.connecting'),
        description: t('providers.connectDesc'),
      })
      await window.electronAPI.webRuntime.connect(providerId, accountId)
      toast({
        title: t('providers.connectSuccess'),
        description: t('providers.connectSuccessDesc'),
      })
      // Health-check the sidecar so the card status reflects the connected session.
      await window.electronAPI.webRuntime.health().catch(() => null)
    } catch (error) {
      toast({
        title: t('providers.connectFailed'),
  ...
  /** Web provider: scan Chrome/Edge cookies and import ChatGPT / Doubao sessions. */
  const handleWebImportBrowser = async (
    providerId: string,
    browser: WebProviderBrowser
  ): Promise<void> => {
    if (webActionDialog) {
      setWebActionDialog({ ...webActionDialog, busy: true, busyLabel: t('providers.importing') })
    }

    try {
      toast({
        title: t('providers.importing'),
        description: t('providers.importBrowserDesc'),
      })

      const imported = await window.electronAPI.webRuntime.importBrowser(browser)
      const relevant = imported.filter(
        (s) => s.providerId === 'chatgpt' || s.providerId === 'doubao'
      )
      const provider = store.getProviderById(providerId)
      const webProvider = providerId.replace(/-web$/, '')
      const matched = provider ? relevant.filter((s) => s.providerId === webProvider) : []

      let persisted = 0
      if (provider && matched.length > 0) {
      ...
      if (persisted > 0) {
        toast({
          title: t('providers.importSuccess'),
          description: t('providers.importSuccessDesc', { count: persisted }),
        })
      } else {
        toast({
          title: t('providers.importFailed'),
          description: t('providers.importFailedDesc'),
          variant: 'destructive',
        })
      }
    } catch (error) {
      toast({
        title: t('providers.importFailed'),
        description: error instanceof Error ? error.message : t('providers.importFailedDesc'),
        variant: 'destructive',
      })
    } finally {
      setWebActionDialog(null)
  ...
  const handleSelectBuiltinProvider = async (provider: BuiltinProviderConfig, credentials: Record<string, string>) => {
    ...
    // A web provider has no token to save at this step. The action dialog
    // opens immediately after the provider is registered so Connect can
    // launch the managed Chromium login flow.
    if (provider.type === 'web') {
      setShowAddProviderDialog(false)
      handleWebActionClick(provider.id, 'connect')
      return
    }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid gap-4 pr-4">
            {filteredProviders.map((provider) => (
              <ProviderCard
                ...
                onConnect={(id) => handleWebActionClick(id, 'connect')}
                onImportBrowser={(id) => handleWebActionClick(id, 'importBrowser')}
      ...
      <WebProviderActionsDialog
        open={!!webActionDialog}
        onOpenChange={closeWebActionDialog}
        action={webActionDialog?.action || null}
        providerName={webActionProviderName}
        isBusy={webActionDialog?.busy || false}
        busyLabel={webActionDialog?.busyLabel}
        onConnect={() => webActionDialog && handleWebConnect(webActionDialog.providerId)}
        onImportBrowser={(browser) =>
          webActionDialog && handleWebImportBrowser(webActionDialog.providerId, browser)
        }
        onManualToken={() => {
          if (!webActionDialog) return
          const providerId = webActionDialog.providerId
          setWebActionDialog(null)
          handleManageAccounts(providerId)
          setShowAddAccountDialog(true)
        }}
      />
    </div>
