...
interface ProviderCardProps {
  ...
  onImportBrowser?: (id: string) => void
    ...
    <Card hover className={cn('', className)}>
      ...
      <CardContent>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="grid grid-cols-4 gap-4 text-sm flex-1 min-w-[320px]">
            ...
            <div className="flex flex-col">
              <span className="text-muted-foreground text-xs">{t('dashboard.totalTokens')}</span>
              <span className="font-medium">
                {totalTokens ? `${formatTokenCount(totalTokens)} Token` : '0 Token'}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-muted-foreground text-xs">{t('providers.authType')}</span>
              <span className="font-medium text-xs truncate">
                {getAuthTypeLabel(provider.authType, t)}
              </span>
            </div>
          </div>
          ...
          {isWeb && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => onImportBrowser?.(provider.id)}
                className="shrink-0"
              >
...
function getAuthTypeLabel(authType: string, t?: (key: string) => string): string {
  const labels: Record<string, string> = {
    ...
    browser: t ? t('providers.browser') : 'Browser session',
  }
