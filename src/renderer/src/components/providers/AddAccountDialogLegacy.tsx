        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4 mt-4">
            ...
            {validationResult.logs && validationResult.logs.length > 0 && (
              <div className="rounded-lg border bg-muted/30 p-3 text-xs font-mono space-y-1 max-h-32 overflow-y-auto">
                {validationResult.logs.map((log, index) => (
                  <div key={`${log.timestamp}-${index}`} className={log.level === 'error' ? 'text-red-500' : 'text-muted-foreground'}>
...
function getDefaultCredentialFields(authType?: string, t?: (key: string) => string): CredentialField[] {
  const fieldConfigs: Record<string, CredentialField[]> = {
    ...
    // Web providers (type: 'web', authType: 'browser') have no credentialFields.
    // Fall back to a single optional token / cookie paste box (not required —
    // the user may instead use Connect or browser import).
    browser: [
      {
        name: 'token',
        label: t ? t('providers.accountToken') : 'Token',
        type: 'textarea',
        required: false,
        placeholder: t ? t('providers.accountTokenPlaceholder') : 'Paste token or cookie here',
        helpText: t ? t('providers.accountTokenHelp') : undefined,
      },
    ],
  }
