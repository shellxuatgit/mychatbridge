...
import { getBuiltinProviderNextAction } from './webProviderFlow'
import {
  shouldShowPlaywrightLoginButton,
  credentialsFromPayload,
} from './providersPlaywrightFlow'
...
export function AddProviderDialog({
  ...
  const [pwLoginProviders, setPwLoginProviders] = useState<string[]>([])
  const [pwImporting, setPwImporting] = useState(false)
  const [pwStatus, setPwStatus] = useState('')
  const [pwImportedCookies, setPwImportedCookies] = useState<Array<Record<string, unknown>> | null>(null)
  ...
  useEffect(() => {
    if (!open) return
    let cancelled = false
    window.electronAPI?.webRuntime.playwrightLoginConfig?.().then((config) => {
      if (!cancelled) setPwLoginProviders(Object.keys(config))
    }).catch(() => {})
  ...
  const handleSubmit = async () => {
    ...
    if (selectedProviderData.type === 'web') {
      await onSelectBuiltin(selectedProviderData, {})
      onOpenChange(false)
      setStep(1)
      setSelectedProvider(null)
      setSearchQuery('')
    ...
    if (pwImportedCookies && selectedProviderData) {
      try {
        await window.electronAPI?.webRuntime.saveImportedSession(
          selectedProviderData.id,
          pwImportedCookies
        )
      } catch (error) {
        console.error('[AddProviderDialog] saveImportedSession failed:', error)
      }
    }
  ...
  const handlePlaywrightLogin = async () => {
    if (!selectedProviderData) return
    setPwImporting(true)
    setPwStatus(t('providers.playwrightLogin.waiting'))
    try {
      const result = await window.electronAPI?.webRuntime.playwrightLogin(selectedProviderData.id)
      if (result.status === 'imported') {
        setPwImportedCookies(result.payload.cookies ?? null)
        setCredentials((prev) => ({ ...prev, ...credentialsFromPayload(result.payload) }))
        setPwStatus(t('providers.playwrightLogin.success'))
        setValidationResult({ valid: true })
      } else if (result.status === 'browser-missing') {
        setPwStatus(t('providers.playwrightLogin.browserMissing'))
      } else if (result.status === 'timeout') {
        setPwStatus(t('providers.playwrightLogin.timeout'))
      } else {
        setPwStatus(t('providers.playwrightLogin.cancelled'))
      }
    } catch (error) {
      setPwStatus(error instanceof Error ? error.message : t('providers.playwrightLogin.timeout'))
    } finally {
      setPwImporting(false)
    }
  }

  const handleCancelPlaywrightLogin = async () => {
    if (!selectedProviderData) return
    await window.electronAPI?.webRuntime.cancelPlaywrightLogin?.(selectedProviderData.id)
  }
  ...
  const handleNextStep = () => {
    ...
    if (getBuiltinProviderNextAction(selectedProviderData || {}) === 'connect') {
      setStep(2)
      setActiveTab('manual')
      return
    }
      ...
      <div className="border-t pt-4">
        {selectedProviderData?.type === 'web' ? (
          <div className="rounded-md border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-100">
            <p className="font-medium">{t('providers.connect')}</p>
            <p className="mt-1 text-xs opacity-80">{t('providers.connectDesc')}</p>
            <p className="mt-2 text-xs opacity-80">{t('providers.connectNextHint')}</p>
          </div>
        ) : (
          <>
        ...
        {shouldShowPlaywrightLoginButton(selectedProviderData?.id || '', pwLoginProviders) && (
          <div className="mt-4 flex items-center gap-2">
            <Button
              ...
              disabled={pwImporting}
            >
              {pwImporting ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  {pwStatus || t('providers.playwrightLogin.waiting')}
                </>
              ) : (
                t('providers.playwrightLogin.button')
              )}
            </Button>
            {pwImporting && (
              <Button type="button" variant="ghost" size="sm" onClick={handleCancelPlaywrightLogin}>
                {t('providers.playwrightLogin.cancel')}
              </Button>
            )}
          </div>
        )}
        {pwStatus && !pwImporting && (
          <p className={`mt-2 text-xs ${pwStatus === t('providers.playwrightLogin.success') ? 'text-green-600' : 'text-muted-foreground'}`}>
            {pwStatus}
          </p>
        )}
