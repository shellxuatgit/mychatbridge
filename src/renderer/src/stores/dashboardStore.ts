...
interface DashboardState {
  ...
  setTokenTrend: (trend: { date: string; tokens: number }[]) => void
...
export const useDashboardStore = create<DashboardState>((set, get) => ({
  ...
  refreshData: async () => {
    const { setLoading, setError, setProxyStatus, setStatistics, setStats, setProviders, setActivities, setChartData, setTokenTrend } = get()
    ...
    try {
      ...
      const providerTokensMap = persistentStats?.providerTokens ?? {}
      const totalTokens = Object.values(providerTokensMap).reduce((sum: number, u: any) => sum + (u?.totalTokens ?? 0), 0)
      const todayTokensMap = todayStats?.providerTokens ?? {}
      const todayTokens = Object.values(todayTokensMap).reduce((sum: number, u: any) => sum + (u?.totalTokens ?? 0), 0)

      const tokenTrend: { date: string; tokens: number }[] = []
      for (let i = 6; i >= 0; i--) {
        const d = new Date(Date.now() - i * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
        const dayTokens = persistentStats?.dailyStats?.[d]?.providerTokens ?? {}
        tokenTrend.push({
          date: d.slice(5),
          tokens: Object.values(dayTokens).reduce((sum: number, u: any) => sum + (u?.totalTokens ?? 0), 0),
        })
      }
      ...
      const providerStats: ProviderStats[] = (providers ?? []).map((provider: Provider) => {
        ...
        return {
          ...
          totalTokens: providerTokensMap[provider.id]?.totalTokens ?? 0,
        }
      })
