...
export type Language = 'zh-CN' | 'en-US' | 'zh-TW' | 'ja-JP' | 'ko-KR' | 'es-ES' | 'fr-FR' | 'de-DE' | 'ru-RU'
...
export function getDefaultLanguage(): Language {
  ...
  if (language.includes('zh-tw') || language.includes('zh-hk') || language.includes('zh-mo')) return 'zh-TW'
  if (language.includes('zh')) return 'zh-CN'
  if (language.includes('ja')) return 'ja-JP'
  if (language.includes('ko')) return 'ko-KR'
  if (language.includes('es')) return 'es-ES'
  if (language.includes('fr')) return 'fr-FR'
  if (language.includes('de')) return 'de-DE'
  if (language.includes('ru')) return 'ru-RU'
  return 'en-US'
}
      ...
      fetchConfig: async () => {
        try {
          ...
          set({ 
            ...
            language: config.language || get().language || getDefaultLanguage(),
          })
        } catch (error) {
