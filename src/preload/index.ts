...
const proxyAPI = {
  ...
  testConnection: (options?: { model?: string; apiKey?: string }) =>
    ipcRenderer.invoke(IpcChannels.PROXY_TEST_CONNECTION, options),
...
/** A session imported from a local browser cookie store (chatgpt / doubao). */
...
type AutoConnectResult =
  | { status: 'imported'; payload: { providerId: string; credentials: Record<string, string>; accountName: string } }
  | { status: 'opened' }
...
type PlaywrightLoginResult =
  | { status: 'imported'; payload: { providerId: string; credentials: Record<string, string>; accountName: string; cookies: Array<Record<string, unknown>> } }
  | { status: 'cancelled' }
  | { status: 'timeout' }
...
const webRuntimeAPI = {
  connect: (providerId: string, accountId: string): Promise<{ ok: true }> =>
    ipcRenderer.invoke('webRuntime:connect', { providerId, accountId }),

  importBrowser: (browser?: 'chrome' | 'edge'): Promise<ImportedSession[]> =>
    ipcRenderer.invoke('webRuntime:importBrowser', { browser }),

  autoConnect: (providerId: string): Promise<AutoConnectResult> =>
    ipcRenderer.invoke('webRuntime:autoConnect', providerId),

  checkSession: (providerId: string): Promise<CheckSessionResult> =>
    ipcRenderer.invoke('webRuntime:checkSession', providerId),

  health: (): Promise<unknown> =>
    ipcRenderer.invoke('webRuntime:health'),

  playwrightLoginConfig: (): Promise<Record<string, { loginUrl: string }>> =>
    ipcRenderer.invoke('webRuntime:playwrightLoginConfig'),

  playwrightLogin: (providerId: string): Promise<PlaywrightLoginResult> =>
    ipcRenderer.invoke('webRuntime:playwrightLogin', { providerId }),

  cancelPlaywrightLogin: (providerId: string): Promise<{ ok: true }> =>
    ipcRenderer.invoke('webRuntime:cancelPlaywrightLogin', { providerId }),

  saveImportedSession: (providerId: string, cookies: Array<Record<string, unknown>>): Promise<{ accountId: string }> =>
    ipcRenderer.invoke('webRuntime:saveImportedSession', { providerId, cookies }),
}
...
interface PersistentStatistics {
  ...
  providerTokens: Record<string, { promptTokens: number; completionTokens: number; totalTokens: number }>
...
const statisticsAPI = {
  ...
  onUpdated: (callback: (stats: PersistentStatistics) => void): (() => void) => {
    const handler = (_: unknown, stats: PersistentStatistics) => callback(stats)
    ipcRenderer.on(IpcChannels.STATISTICS_UPDATED, handler)
    return () => ipcRenderer.removeListener(IpcChannels.STATISTICS_UPDATED, handler)
  },
}
...
const appAPI = {
  ...
  simulateUpdate: (targetVersion?: string): Promise<UpdateStatus> =>
    ipcRenderer.invoke(IpcChannels.APP_SIMULATE_UPDATE, targetVersion),
