...
export interface TestLogLine {
  timestamp: number
  level: 'info' | 'error'
  /** Pre-translated fallback (zh-CN). Kept for backwards compat / persisted logs. */
  message: string
  /** i18n key (e.g. 'providers.testLogs.start'). Renderer translates if present. */
  messageKey?: string
...
export interface StatisticsResponse {
  ...
  providerUsage: Record<string, number>
  accountUsage: Record<string, number>
  /** Token usage per provider (all time) */
  providerTokens?: Record<string, TokenUsageSummary>
