...
MyChatBridge Manager is an Electron desktop application that provides an OpenAI-compatible API proxy for multiple AI service providers (DeepSeek, GLM, Kimi, MiniMax, Qwen, Z.ai, Perplexity). It enables using any OpenAI-compatible client with these providers across macOS, Windows, and Linux.
...
Application data is stored in `~/.mychatbridge/`:
...
> **Gateway abstractions (new in v1.5+):** providers can now be written against the
> unified `WebProviderRuntime` contract in `src/main/proxy/core/providerContract.ts`
> and registered through `src/main/proxy/core/providerRegistry.ts`. Legacy adapters
> are wrapped automatically by `ProviderAdapterBridge` — new providers should
> implement `WebProviderRuntime` directly and register via `registerRuntimeFactory`
> to skip the forwarder-specific plumbing (still add the `forward<Provider>()` method
> to `forwarder.ts` for the legacy dispatch registry).
...
#### 2. OAuth Authentication Layer (Required for OAuth-based providers)
...
| `src/main/proxy/core/capabilities.ts` | Add the provider's default capability set |
| `src/main/proxy/core/providerRegistry.ts` | (optional) register a direct runtime factory |
